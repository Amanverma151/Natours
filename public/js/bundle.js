...
var displayMap = function displayMap(locations) {
  mapboxgl.accessToken = "pk.eyJ1Ijoid2lja2VkeG8iLCJhIjoiY2wyOHkyZ3g1MDByMzNicXVhem1janhoYyJ9.94D8sX89g9S51jNPWBzRZQ";
  var map = new mapboxgl.Map({
    container: "map",
    style: "mapbox://styles/wickedxo/cl290wa0x003l15qlatq8dezg",
    scrollZoom: false // center: [-118.110531, 34.05501],
    // zoom: 10,
  ...
  locations.forEach(function (loc) {
    ...
    el.className = "marker"; // Adding the marker

    new mapboxgl.Marker({
      element: el,
      anchor: "bottom" // means that the bottom of the pin will the exact location on the GPS.

    }).setLngLat(loc.coordinates).addTo(map); // Adding Popup

    new mapboxgl.Popup({
      offset: 40 // To avoid the overlapping, 40px

    }).setLngLat(loc.coordinates).setHTML("<p> ".concat(loc.day, ": ").concat(loc.description, " </p>")).addTo(map); // Extends map bounds to include current location
...
var hideAlert = function hideAlert() {
  var el = document.querySelector(".alert"); // Moving one level up to the parentElement and removing the child element
...
var showAlert = function showAlert(type, msg) {
  ...
  hideAlert();
  var markup = "<div class=\"alert alert--".concat(type, "\"> ").concat(msg, " </div>"); // this will be inserted in the body right after the beginning of the alert
        ...
        switch (_context.prev = _context.next) {
          ...
          case 3:
            ...
            if (res.data.Status === "Success") {
              (0, _alerts.showAlert)("success", "Logged in Successully");
      ...
      while (1) {
        switch (_context.prev = _context.next) {
          ...
          case 3:
            ...
            if (res.data.Status === "Success") {
              (0, _alerts.showAlert)("success", "Signed up Successully");
              window.setTimeout(function () {
                location.assign("/"); // Directing to the home page in 1 sec.
              }, 1000);
            }
...
var stripe = Stripe("pk_test_51KtZ7FSC43EeA5LqHaMAEM8Wouv0mEzDbuJ5bWaeteqcDoHrmDphwncJXSMx71DfkTpV1QM2mp6666dyhaP8bh8c00sAdQxozd"); // this tourId is coming from the pug template where we declare the data property
      ...
      while (1) {
        switch (_context.prev = _context.next) {
          ...
          case 3:
            ...
            console.log(session); // 2. Create a checkout form + charge credit card
...
if (signupForm) loginForm.addEventListener("submit", function (event) {
  event.preventDefault(); // to prevent the form to load any other page.
...
if (userDataForm) userDataForm.addEventListener("submit", function (e) {
  e.preventDefault(); // For taking the form inputs and appending it
    ...
    return regeneratorRuntime.wrap(function _callee$(_context) {
      while (1) {
        switch (_context.prev = _context.next) {
          case 0:
            e.preventDefault();
            document.querySelector(".btn--save-password").textContent = "Updating..."; // these variable names should be the same as in your API

            passwordCurrent = document.getElementById("password-current").value;
            password = document.getElementById("password").value;
            passwordConfirm = document.getElementById("password-confirm").value; // awaiting the promise to clear the input fields after the reload
